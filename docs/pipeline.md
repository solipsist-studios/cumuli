...
Scripts are numbered in pipeline order under `scripts/`. Each script's
own `--help` / docstring has the full flag reference; this doc is the
narrative walkthrough and the conda envs each stage needs.
...
`scripts/21_run_unified_pipeline.py` runs stages 1-18 (plus 20, when a
known-good sync is reused) end-to-end in one process -- it dispatches
each stage into the right conda env itself, so you don't need to
`conda activate` between steps the way the walkthrough below shows.
Read stages 1-20 below first to understand what each one actually does
and why; use the orchestrator to run them without the manual bookkeeping.
...
- **Sync: verify once, reuse, don't re-trust automatically every run.**
  Stage 1's only sync method, raw audio cross-correlation
  (`01_compute_sync_offsets.py`, always runs unless `--initial_sync_json`
  is given), hasn't been reliable enough in real-world testing to trust
  unverified -- it can silently converge on a wrong offset for
  periodic/rhythmic audio. (Automatic skeleton-based correction,
  `19_skeleton_sync_search.py`, isn't part of this build -- it has not
  produced good results in practice; stage 1 just produces the QA grid
  below for manual verification.) The workflow
  that's actually worked every time this session:
  1. Run stage 1 once, inspect stage 03's sync grid by eye, hand-tune
     any camera's `frame_offset` that's visibly off, save the result
     (e.g. `sync_offsets_v5.json`).
  ...
  3. For every run after that on the same take (different timestamps,
     retries, etc.), skip stage 1 entirely: write the verified file's
     path into `<out_dir>/resolved_sync_json.txt` yourself and pass
     `--start_from_stage production`. This is the normal way to iterate
     without re-running (and re-trusting) sync search each time.
- **`--no_diffuman` / `--use_diffuman`** (required, mutually exclusive) --
  branch A (direct 4K masked training on the real cameras, stages
  17+16, the only branch validated so far) vs. branch B (Diffuman4D's
  48-camera dense ring, stages 10-16). **Branch B isn't part of this
  build yet** -- selecting `--use_diffuman` fails immediately with a
  clear message; see "Stages 10-14" below.
...
## 1-3. Sync and extract a frame

```bash
python3 scripts/01_compute_sync_offsets.py \
...
# visually checking 03_make_sync_grid.py output at a few timestamps, and
# save as sync_offsets_v2.json, v3.json, etc. Point everything below at
# whichever version is your current best.

python3 scripts/02_extract_synced_frames.py \
    /media/ai/datasets/260521-105422/movies \
    ~/heidi_260521_undist/sync_offsets_v5.json \
    ~/heidi_1500ms/raw \
    1.5

python3 scripts/03_make_sync_grid.py ~/heidi_1500ms/raw ~/heidi_1500ms/sync_grid.jpg
...
```bash
python3 scripts/04_undistort_frames.py \
...
## 5. HLOC pose estimation

```bash
conda activate hloc
python3 scripts/05_run_hloc.py \
...
## 6. Build the flat, 2-digit-labeled dataset
...
```bash
python3 scripts/06_build_flat_dataset.py \
...
## 7-9. Masks, 2D keypoints, split

```bash
conda activate diffuman4d
python3 scripts/07_generate_masks.py \
    --images_dir ~/heidi_1500ms/images_flat \
    --out_fmasks_dir ~/heidi_1500ms/fmasks_flat

conda activate sapiens2
python3 scripts/08_predict_keypoints_2d.py \
    --images_dir ~/heidi_1500ms/images_flat \
    --out_kp2d_dir ~/heidi_1500ms/poses_2d_flat \
    --fmasks_dir ~/heidi_1500ms/fmasks_flat

python3 scripts/09_split_keypoints_per_camera.py \
    --predictions_json ~/heidi_1500ms/poses_2d_flat \
    --out_dir ~/heidi_1500ms/poses_2d
```

**Stages 10-15 below (the Diffuman4D dense-ring branch) are not yet
included in this build -- only the direct 4K masked branch (`--no_diffuman`
/ stages 17+16) has actually been run and validated.** The scripts and
orchestrator wiring for this branch will land once it's been tested
end-to-end. Kept here as a design reference for that branch's intended
shape.

## 10. Resize for Diffuman4D
...
```bash
python3 scripts/10_resize_for_diffuman4d.py \
    --images_dir ~/heidi_1500ms/images_flat \
    --fmasks_dir ~/heidi_1500ms/fmasks_flat \
...
## 11. Generate the 48-camera ring
...
```bash
python3 scripts/11_generate_camera_ring.py \
    --transforms ~/heidi_1500ms_1024/transforms.json \
    --out_transforms ~/heidi_1500ms_1024/transforms_48cam.json
```

## 12-13. Triangulate, project, draw skeletons

```bash
python3 scripts/12_triangulate_and_project_keypoints.py \
...
python3 scripts/13_draw_skeletons.py \
    --kp2d_dir ~/heidi_1500ms_1024/poses_2d_proj \
    --out_kpmap_dir ~/heidi_1500ms_1024/skeletons
```

## 14. Diffuman4D inference

```bash
conda activate diffuman4d
python3 scripts/14_run_diffuman4d_inference.py \
...
## 15-16. Prepare and train Brush

`15_prepare_brush_dataset.py` (below) is diffuman-branch-only -- it wraps
Diffuman4D's own converter and requires stage 14's inference output, so
it's not usable independent of that branch either; not part of this
build for the same reason as stages 10-14. `16_train_brush.py` (also
below) is shared and validated -- the direct branch uses it via
`17_build_colmap_sparse.py`'s output instead (see the "17-20" section
further down).

```bash
python3 scripts/15_prepare_brush_dataset.py \
...
python3 scripts/16_train_brush.py \
...
## 17-20. Quality improvements (optional, recommended for production captures)

These stages came out of a deep-dive on a static-GoPro-rig capture that
had visible ghosting; each fixes a specific measured failure mode. They
are optional relative to 1-16 above (which is enough to get a first
splat), but for anything beyond a quick test, do them in this order:

**Calibration sanity check** happens automatically inside stage 04 now --
if a camera's calibration `image_size` disagrees with the actual frame
resolution by a uniform scale, it is auto-corrected with a loud warning
(this was a real bug: a calibration made at the GoPro 5.3K full-sensor
width, 5568px, silently applied to 5312px-wide video warped every camera
by ~113px of focal). Read those warnings if they appear -- they mean
you're pointing at the wrong calibration source. `--target_pkl_dir` on
stage 04 also enables single-warp mode: native fisheye straight to a
known target pinhole geometry in one resample, instead of
downscale-then-undistort.
...
`--pp3_dir` to stage 02 and it color-corrects each extracted frame before
undistortion. Matters because uncorrected per-GoPro exposure/saturation
differences show up as color noise in the trained splat.

**Multi-instant capture, sync search, and pose refinement** -- audio sync
(stage 01) fails silently at live-music venues (periodic beats create
false correlation peaks; low confidence scores are the tell). The
subject's own skeleton is the intended fix for that via
`19_skeleton_sync_search.py` (steps below) -- **but that script isn't
...
for when it's validated and re-added. The **pose
refinement** step further below (`20_refine_poses_with_keypoints.py`) is
a different thing -- camera *pose* accuracy, not *sync* timing -- and
...
# Run 04 (undistort) + 07 (masks) + 08 (keypoints) on each instant subdir f0/..f4/
for k in 0 1 2 3 4; do
    python3 scripts/04_undistort_frames.py --frames_dir ~/capture/sync_candidates/f$k \
    ...
    conda activate sapiens_lite  # or sapiens2, see 08's docstring
    python3 scripts/08_predict_keypoints_2d.py \
        --images_dir ~/capture/sync_candidates_undist/f$k \
...
python3 scripts/19_skeleton_sync_search.py \
...
# where background SfM features from stage 05 don't reach)
python3 scripts/20_refine_poses_with_keypoints.py \
    --transforms ~/capture/solipsist_out/transforms_multiframe.json \
    --kp2d_dirs ~/capture/sync_candidates_poses2d/f0,~/capture/sync_candidates_poses2d/f1,~/capture/sync_candidates_poses2d/f2,~/capture/sync_candidates_poses2d/f3,~/capture/sync_candidates_poses2d/f4 \
...
Use `transforms_refined.json` in place of `transforms_multiframe.json` in
every stage from 06 onward. On the capture this was developed against,
this took subject-space median reprojection error from ~53px to ~5px and
fixed between-camera midpoint renders that had shown a dark ghost/fog
around the subject.

**Masked (subject-only) training**: generate masks (stage 07), clean them
before trusting them for anything -- raw BiRefNet output can include
bystanders/props and can also drop part of the subject:

```bash
python3 scripts/18_clean_masks.py \
...
```bash
python3 scripts/17_build_colmap_sparse.py \
...
These stages had no surviving ground-truth command from prior runs and
are first-draft designs -- validate them against a real run before
trusting the output:

- **11_generate_camera_ring.py**: ring geometry (center/up/radius/height
...
- **10_resize_for_diffuman4d.py**: assumes a center crop. If the subject
