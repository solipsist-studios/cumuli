...
NOSE_KP, LEFT_EYE_KP, RIGHT_EYE_KP = 0, 1, 2  # goliath308 face keypoints, as in render_orbit_views.py
...
def face_normal(keypoints: np.ndarray, valid: np.ndarray, up: np.ndarray) -> np.ndarray | None:
    ...
    render_orbit_views.facing_azimuth does, and it is deliberately NOT
...
def resolve_kp3d(frame_dir: Path, kp3d_subdir: str, tem_label: str, kp3d_dir: Path | None,
    ...
    matching the per-frame layout render_and_repair_sequence.py works in. With an
