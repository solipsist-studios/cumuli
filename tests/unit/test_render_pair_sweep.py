...
def make_camera(label, azimuth_deg, elevation_deg=5.0, radius=4.0, focal=1200.0, size=1600):
    ...
    center = rps.position_at(TARGET, UP, E1, E2, azimuth_deg, elevation_deg, radius)
    w2c = rps.lookat_w2c(center, TARGET, -UP)
...
def test_spherical_and_position_at_round_trip_exactly():
    ...
    for _ in range(20):
        ...
        azimuth, elevation, radius = rps.spherical(center, TARGET, UP, E1, E2)
        assert np.allclose(rps.position_at(TARGET, UP, E1, E2, azimuth, elevation, radius), center)
...
def test_shortest_arc(start, end, expected):
    assert rps.shortest_arc(start, end) == pytest.approx(expected)
...
def test_shortest_arc_at_exactly_half_a_turn_picks_one_direction():
    ...
    assert abs(rps.shortest_arc(0.0, 180.0)) == pytest.approx(180.0)
...
def test_shortest_arc_never_takes_the_long_way():
    ...
    for _ in range(50):
        ...
        assert abs(rps.shortest_arc(start, end)) <= 180.0 + 1e-9
...
def test_sweep_endpoints_land_on_the_real_camera_centres():
    ...
    assert np.allclose(rps.position_at(TARGET, UP, E1, E2, *parameters[0]), a["center"])
    assert np.allclose(rps.position_at(TARGET, UP, E1, E2, *parameters[-1]), b["center"])
...
def test_probe_frame_is_snapped_to_the_holdout_camera_exactly():
    ...
    assert np.allclose(rps.position_at(TARGET, UP, E1, E2, *parameters[probe]), holdout["center"])
...
def test_homography_maps_the_frustum_back_to_the_real_image_exactly():
    ...
    w2c_new = rps.lookat_w2c(camera["center"], TARGET, -UP)
